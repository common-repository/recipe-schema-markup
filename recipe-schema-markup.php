...
echo <<<HTML
    <div class="blog-buymeapie-recipe {$theme->layout->style} {$condensed} {$numberedIngredients} {$numberedMethods} {$numberedNotes}" itemscope itemtype="http://schema.org/Recipe" id="recipe-{$recipeId}">
HTML;
...
HTML;
		...
		if (!empty($section->lines)) {
			foreach ($section->lines as $line) {
				$line = explode('|', $line);
				$line = '<span itemprop="name">' . $line[0] . '</span> <span itemprop="amount">' . $line[1] . '</span>';
echo <<<HTML
...
?>	
	...
	<div class="blog-buymeapie-import blog-buymeapie-action" data-recipe-id="recipe-<?php echo $recipeId ?>"><span><div><?php echo $import_recipe_button_text ?></div></span></div>
...
function buymeapie_recipe_wp_head() {
	...
	$bmap_lang = get_locale() == 'ru_RU' ? 'ru' : 'en';
